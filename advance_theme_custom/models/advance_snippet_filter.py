...
class WebsiteSnippetFilter(models.Model):
    ...
    def _get_categories(self, mode='all', limit=None, **kwargs):
        """Devuelve categorías publicadas"""
        website = self.env['website'].get_current_website()
        domain = [('website_published', '=', True)]
        categories = self.env['product.public.category'].search(domain, limit=limit or self.limit)
    ...
    def _get_hardcoded_sample(self, model):
        ...
        if model._name == 'product.public.category':
            data = [
                {'display_name': _("Chairs"), 'image_512': b'/product/static/img/category_chairs.jpg'},
                {'display_name': _("Lamps"), 'image_512': b'/product/static/img/category_lamps.jpg'},
                {'display_name': _("Tables"), 'image_512': b'/product/static/img/category_tables.jpg'},
            ]
            samples = data
        return samples
